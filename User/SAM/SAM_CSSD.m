function [varargout]=SAM_CSSD(varargin)
if nargin==2
    y=varargin{1};
    Fs=varargin{2};
    Times=inf;
...
% yy(L+1:L+N)=y;
% y=yy;
%% 只运行SAM_Cao_SSD  检验嵌入维数自适应的优点
ssc=SAM_Cao_SSD(y,Fs,Times,draws);
%% 剔除多余数据
% ssc=ssc(:,L+1:L+N);
if nargout>0
    varargout{1}=ssc;
...
%%                     改进     
                    if Fs/ff(in3)<=100   %处理高频分量  要提高嵌入维数(选值必须大于33)
                        if ceil(10*Fs/ff(in3))- ceil(Fs/(ff(in3)*10))<100                    %防止处理低频分量时 数据点不够
                            a_m=ceil(0.5*Fs/ff(in3));
                            b_m=1;
                            c_m=a_m+99;
                        else
                            a_m=ceil(Fs/(ff(in3)*10));
                            b_m=ceil((ceil(10*Fs/ff(in3))-ceil(Fs/(ff(in3)*10)))/100);
                            c_m=ceil(10*Fs/ff(in3));
                        end

                    else              %处理低频分量  要降低嵌入维数
                        a_m=ceil(Fs/ff(in3));
                        c_m=ceil(Fs/ff(in3)*2);
                        b_m=ceil((c_m-a_m)/100);
                    end
                     [l,DD1,dd1]=SA_Caos(v2,[a_m,b_m,c_m]);
%                     a_m=ceil(Fs/(ff(in3)*10));
%                     b_m=ceil((ceil(10*Fs/ff(in3))-ceil(Fs/(ff(in3)*10)))/50);
%                     c_m=ceil(10*Fs/ff(in3));
%                     if c_m>=floor(L/3)
%                         c_m=floor(L/3);
%                     end
